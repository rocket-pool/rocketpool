...
export async function scenarioCreateMinipool({nodeOwner, durationID, validatorPubkey, validatorSignature, validatorDepositDataRoot, etherAmount, rplAmount, isTrusted, fromAddress, gas}) {
    const rocketPool = await RocketPool.deployed();

    // Create minipool
    await rocketPool.minipoolCreate(nodeOwner, durationID, validatorPubkey, validatorSignature, validatorDepositDataRoot, etherAmount, rplAmount, isTrusted, {from: fromAddress, gas: gas});
