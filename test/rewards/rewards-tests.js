...
export default function() {
    contract('RocketRewardsPool', async (accounts) => {
        ...
        it(printTitle('node', 'cannot claim RPL while their node is undercollateralized'), async () => {
            ...
            const calculatedTotalEffectiveStake = await getCalculatedTotalEffectiveRPLStake();
            await submitPrices(10, web3.utils.toWei('0.01', 'ether'), calculatedTotalEffectiveStake, {from: registeredNodeTrusted1});
            await submitPrices(10, web3.utils.toWei('0.01', 'ether'), calculatedTotalEffectiveStake, {from: registeredNodeTrusted2});
