...
import { claimRewards, claimRewardsUsingClaimerAddress } from "./scenario-claim-rewards";
  ...
  contract.only("RocketRewardsPool", async (accounts) => {
    ...
    let rplInflationSetup = async function () {
      ...
      assert.isAbove(
        claimIntervalTime,
        ONE_DAY,
        "Tests will not function correctly unless claimIntervalTime is greater than inflation period (1 day)"
      );
    ...
    before(async () => {
      ...
      // Set settings
      await setDAONodeTrustedBootstrapSetting(
        RocketDAONodeTrustedSettingsMinipool,
    ...
    it(printTitle("userOne", "fails to set interval blocks for rewards claim period"), async () => {
      ...
      await shouldRevert(
        setRewardsClaimIntervalTime(100, {
          from: userOne,
        }),
    ...
    it(printTitle("userOne", "fails to set contract claimer percentage for rewards"), async () => {
      // Set the amount this contract can claim
      await shouldRevert(
        setDAONetworkBootstrapRewardsClaimers("0.1".ether, "0.5".ether, "0.4".ether, {
          from: userOne,
        }),
    ...
    it(printTitle("guardian", "fails to set contract claimer percentages to lower than 100% total"), async () => {
      // Set the amount this contract can claim
      await shouldRevert(
        setDAONetworkBootstrapRewardsClaimers("0.1".ether, "0.1".ether, "0.1".ether, {
          from: owner,
        }),
    ...
    it(printTitle("guardian", "fails to set contract claimer percentages to greater than 100% total"), async () => {
      // Set the amount this contract can claim
      await shouldRevert(
        setDAONetworkBootstrapRewardsClaimers("0.4".ether, "0.4".ether, "0.4".ether, {
          from: owner,
        }),
        "Percentages were updated",
        "Total does not equal 100%"
    ...
    it(printTitle("node", "can claim RPL and ETH"), async () => {
      ...
      // Set node 2 withdrawal addresses
      await setNodeWithdrawalAddress(registeredNode2, node2WithdrawalAddress, { from: registeredNode2 });
      await setNodeRPLWithdrawalAddress(registeredNode2, node2RPLWithdrawalAddress, { from: node2WithdrawalAddress });
      ...
      const rewards2 = [
        ...
        {
          address: node2WithdrawalAddress,
          network: 0,
          trustedNodeRPL: "0".ether,
          nodeRPL: "0".ether,
          nodeETH: "1".ether,
        ...
        {
          address: node2RPLWithdrawalAddress,
          network: 0,
          trustedNodeRPL: "0".ether,
          nodeRPL: "2".ether,
          nodeETH: "0".ether,
      ...
      // Do a third claim interval, after the changes
      await submitRewards(2, rewards2, "0".ether, "0".ether, { from: registeredNodeTrusted1 });
      ...
      await shouldRevert(
        claimRewards(node2WithdrawalAddress, [2], [rewards2], {
          from: node2RPLWithdrawalAddress,
        })
      );
      // Can't claim RPL rewards from ETH withdrawal address
      await shouldRevert(
        claimRewards(node2RPLWithdrawalAddress, [2], [rewards2], {
          from: node2WithdrawalAddress,
        })
      );
      // Can't claim RPL rewards from node address
      await shouldRevert(
        claimRewards(node2RPLWithdrawalAddress, [2], [rewards2], {
          from: registeredNode2,
        })
      );
      await claimRewards(node2WithdrawalAddress, [2], [rewards2], {
        from: node2WithdrawalAddress,
      });
      await claimRewards(node2RPLWithdrawalAddress, [2], [rewards2], {
        from: node2RPLWithdrawalAddress,
      });
    });
    ...
    it(printTitle("node", "can not claim with invalid proof"), async () => {
      ...
      await shouldRevert(
        rocketMerkleDistributorMainnet.claim(registeredNode2, [0], amountsRPL, amountsETH, proofs, {
          from: registeredNode2,
        }),
        "Was able to claim with invalid proof",
        "Invalid proof"
      );
    });
    ...
    it(printTitle("node", "can not claim same interval twice"), async () => {
      ...
      await shouldRevert(
        claimRewards(registeredNode1, [0], [rewards], {
          from: node1WithdrawalAddress,
        }),
        "Was able to claim again",
        "Already claimed"
      );
      await shouldRevert(
        claimRewards(registeredNode1, [1], [rewards], {
          from: node1WithdrawalAddress,
        }),
        "Was able to claim again",
        "Already claimed"
      ...
      await shouldRevert(
        claimRewards(registeredNode1, [0, 1], [rewards, rewards], {
          from: node1WithdrawalAddress,
        }),
        "Was able to claim again",
        "Already claimed"
      ...
      await shouldRevert(
        claimRewards(registeredNode1, [0, 2], [rewards, rewards], {
          from: node1WithdrawalAddress,
        }),
        "Was able to claim again",
        "Already claimed"
    ...
    it(printTitle("node", "can claim both RPL & ETH if RPL withdrawal is set"), async () => {
        // Initialize RPL inflation & claims contract
        let rplInflationStartTime = await rplInflationSetup();
        await rewardsContractSetup("0.5".ether, "0".ether, "0.5".ether);
        ...
        assert.isBelow(currentTime, rplInflationStartTime, "Current block should be below RPL inflation start time");
        await increaseTime(web3, rplInflationStartTime - currentTime + claimIntervalTime);
        await setNodeRPLWithdrawalAddress(registeredNode2, node2RPLWithdrawalAddress, { from: registeredNode2 });
        // Send ETH to rewards pool
        const rocketSmoothingPool = await RocketSmoothingPool.deployed();
        await web3.eth.sendTransaction({ from: owner, to: rocketSmoothingPool.address, value: "20".ether });
        // Submit rewards snapshot
        ...
        // Claim both ETH and RPL
        await claimRewards(registeredNode2, [0], [rewards], {
          from: node2RPLWithdrawalAddress,
        });
      });

    it(printTitle("node", "can only claim and stake if no RPL withdrawal address set"), async () => {
      // Initialize RPL inflation & claims contract
      let rplInflationStartTime = await rplInflationSetup();
      await rewardsContractSetup("0.5".ether, "0".ether, "0.5".ether);
      ...
      await shouldRevert(claimAndStakeRewards(node2RPLWithdrawalAddress, [0], [rewards], "2".ether, {
        from: node2RPLWithdrawalAddress,
      }));
    });
    ...
    it(printTitle("node", "can not stake amount greater than claim"), async () => {
      ...
      // Claim RPL
      await shouldRevert(
        claimAndStakeRewards(registeredNode1, [0], [rewards], "2".ether, {
          from: registeredNode1,
        }),
        "Was able to stake amount greater than reward",
    ...
    it(printTitle("random", "can execute reward period if consensus is reached"), async () => {
      ...
      await kickTrustedNode(unregisteredNodeTrusted1, [
        registeredNodeTrusted1,
        registeredNodeTrusted2,
        unregisteredNodeTrusted1,
      ]);
    ...
    it(printTitle("misc", "claim bitmap is correct"), async () => {
      ...
      for (let i = 0; i < claimIntervals.length; i++) {
        await shouldRevert(
          claimRewards(registeredNode1, [claimIntervals[i]], [rewards], {
            from: registeredNode1,
          }),
          "Was able to claim again",
          "Already claimed"
