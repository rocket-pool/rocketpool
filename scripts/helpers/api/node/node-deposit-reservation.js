...
module.exports = async (done) => {
    ...
    try {
        ...
        let gasEstimate = await rocketNodeContract.depositReserve.estimateGas(depositAmount, '3m', {
            from: accounts[0]
        })
        // Perform add group
        let result = await rocketNodeContract.depositReserve(depositAmount, '3m', {
            from: accounts[0],
            gas: gasEstimate
        });
        // Show events
        ...
        done('Node contract deposit reservation successfull: ' + args.join(', '));
      } catch (err) {
