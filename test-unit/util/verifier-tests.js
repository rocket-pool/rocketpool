...
import { printTitle } from '../../test-old/_utils/formatting';
import * as assert from 'assert';
import { shouldRevert } from '../../test-old/_utils/testing';
...
export default function() {
    describe('BeaconStateVerifier', () => {
        let beaconStateVerifier;
        let snapshot;

        const farFutureEpoch = 18_446_744_073_709_551_615n;

        // Setup
        before(async () => {
            const factory = await ethers.getContractFactory('BeaconStateVerifierMock');
            beaconStateVerifier = await factory.deploy(ethers.ZeroAddress);
            await beaconStateVerifier.waitForDeployment();
        });
