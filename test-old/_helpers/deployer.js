...
// Register the complete deployment set before the asynchronous Hardhat 3
// artifact initialisation pass runs.
for (const contractName of Object.values(contractNameMap)) {
    artifacts.require(contractName);
}
artifacts.require('RocketStorage');
    ...
    async deployRemainingContracts() {
        for (const contract in this.contractPlan) {
            ...
            try {
                ...
                error.message = `Failed to deploy ${contract}: ${error.message}`;
                throw error;
            }
        }
    }
