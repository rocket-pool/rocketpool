...
contract RocketNodeDeposit is RocketBase, RocketNodeDepositInterface {
    ...
    function deposit(
    ...
    ) external payable override onlyLatestContract('rocketNodeDeposit', address(this)) onlyRegisteredNode(msg.sender) {
        // Load contracts
        ...
        require(
            !rocketMinipoolManager.getMinipoolExists(_expectedMinipoolAddress) &&
                !rocketMinipoolManager.getMinipoolDestroyed(_expectedMinipoolAddress),
            'Minipool already exists or was previously destroyed'
        );
        // Check node fee
        ...
        minipool.nodeDeposit{value: msg.value}(_validatorPubkey, nodeId);
        // Assign deposits if enabled
        assignDeposits();
    }

    // Returns the minipool deposit enum value correseponding to the supplied deposit amount
    function getDepositType(uint256 _amount) public view override returns (MinipoolDeposit) {
        // Get contract
        ...
        if (_amount == rocketDAOProtocolSettingsMinipool.getFullDepositNodeAmount()) {
            return MinipoolDeposit.Full;
        } else if (_amount == rocketDAOProtocolSettingsMinipool.getHalfDepositNodeAmount()) {
            return MinipoolDeposit.Half;
        }
