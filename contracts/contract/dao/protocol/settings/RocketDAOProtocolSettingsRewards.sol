...
contract RocketDAOProtocolSettingsRewards is RocketDAOProtocolSettings, RocketDAOProtocolSettingsRewardsInterface {
    ...
    constructor(RocketStorageInterface _rocketStorageAddress) RocketDAOProtocolSettings(_rocketStorageAddress, "rewards") {
        ...
        if(!getBool(keccak256(abi.encodePacked(settingNameSpace, "deployed")))) {
            // Each of the initial GGP reward claiming contracts
            ...
            // GGP Claims settings
            setSettingUint("ggp.rewards.claim.period.time", 28 days);                                           // The time in which a claim period will span in seconds - 28 days by default
            // Deployment check
    ...
    // Set a new claimer for the ggp rewards, must specify a unique contract name that will be claiming from and a percentage of the rewards
    ...
    // GGP Rewards Claimers (own namespace to prevent DAO setting voting to overwrite them)
