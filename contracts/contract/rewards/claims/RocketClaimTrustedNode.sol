...
contract RocketClaimTrustedNode is RocketBase, RocketRewardsClaimTrustedNodeInterface {
    ...
    function getClaimPossible(address _trustedNodeAddress) override public view onlyTrustedNode(_trustedNodeAddress) returns (bool) {
        ...
        return rewardsPool.getClaimContractRegisteredCanClaim('rocketClaimTrustedNode', _trustedNodeAddress);
    }
    ...
    function getClaimRewardsPerc(address _trustedNodeAddress) override public view onlyTrustedNode(_trustedNodeAddress) returns (uint256) {
        ...
        uint256 trustedNodesClaimIntervalTotal = rewardsPool.getClaimIntervalContractTotalCurrent('rocketClaimTrustedNode');
    ...
    function register(address _trustedNode, bool _enable) override external onlyTrustedNode(_trustedNode) onlyLatestContract("rocketNodeManager", getContractAddress("rocketNodeManager")) {
        // Init the rewards pool contract
        RocketRewardsPoolInterface rewardsPool = RocketRewardsPoolInterface(getContractAddress('rocketRewardsPool'));
        // Register/Unregister now
        rewardsPool.register(_trustedNode, _enable);
    }
    ...
    function claim() override external onlyTrustedNode(msg.sender) {
        ...
        uint256 claimedContractTotal = rewardsPool.getClaimIntervalContractTotalClaimed('rocketClaimTrustedNode');
