...
contract RocketMinipoolDelegate is RocketMinipoolStorageLayout, RocketMinipoolInterface {
    ...
    uint256 constant prelaunchAmount = 3 ether; // The amount of ETH initially deposited when minipool is created
    ...
    event EtherWithdrawnForStaking(address from, uint256 time);
    event EtherWithdrawalProcessed(
        address indexed executed,
        uint256 nodeAmount,
        uint256 userAmount,
    ...
    function getScrubVoted(address _member) external view override returns (bool) {
        return memberScrubVotes[_member];
    }

    // Deposit type getter
    function getDepositType() external view override returns (MinipoolDeposit) {
        return depositType;
    }
    ...
    function getNodeDepositAssigned() external view override returns (bool) {
        return nodeDepositAssigned;
    }

    function getNodeId() external view override returns ( string memory ) {
        return nodeId;
    }
    ...
    modifier onlyMinipoolOwnerOrWithdrawalAddress(address _nodeAddress) {
        require(
            _nodeAddress == nodeAddress || _nodeAddress == rocketStorage.getNodeWithdrawalAddress(nodeAddress),
    ...
    function initialise(address _nodeAddress, MinipoolDeposit _depositType) external override onlyUninitialised {
        // Check parameters
        require(_nodeAddress != address(0x0), 'Invalid node address');
        require(_depositType != MinipoolDeposit.None, 'Invalid deposit type');
        // Load contracts
        RocketNetworkFeesInterface rocketNetworkFees = RocketNetworkFeesInterface(
            getContractAddress('rocketNetworkFees')
        );
        // Set initial status
        ...
        // Set the ggpAVAX address
        gogoTokenGGPAVAX = getContractAddress('gogoTokenGGPAVAX');
    ...
    ) external payable override onlyLatestContract('rocketNodeDeposit', msg.sender) onlyInitialised {
    ...
    onlyInitialised
    {
        ...
        require(
            status >= MinipoolStatus.Initialised && status <= MinipoolStatus.Staking,
            'The user deposit can only be assigned while initialised, in prelaunch, or staking'
        );
    ...
    function refund() external override onlyMinipoolOwnerOrWithdrawalAddress(msg.sender) onlyInitialised {
    ...
    // Called to slash node operator's GGP balance if withdrawal balance was less than user deposit
    ...
    // Called by node operator to finalise the pool and unlock their GGP stake
    ...
    function canStake() external view override onlyInitialised returns (bool) {
        // Check status
...
//        DepositInterface casperDeposit = DepositInterface(getContractAddress('casperDeposit'));
        ...
        require(
            rocketMinipoolManager.getMinipoolByPubkey(_validatorPubkey) == address(0x0),
        ...
        // Send staking deposit to PreStakeVault
//        casperDeposit.deposit{value : prelaunchAmount}(
//            _validatorPubkey,
...
//        );
    ...
    function withdrawBalanceForStaking() external {
        // Only guardian or ODAO node can withdraw
//        require(msg.sender == guardian || getBool(keccak256(abi.encodePacked("dao.trustednodes.", "member", msg.sender))), 'Is not guardian or trusted node');
        // Withdraw contract balance to trusted node or guardian
        address payable to = payable(msg.sender);
        to.transfer(getBalance());
        // Emit ether withdrawn event
        emit EtherWithdrawnForStaking(msg.sender, block.timestamp);
    ...
    external
    override
    onlyLatestContract('rocketMinipoolStatus', msg.sender)
    onlyInitialised
    {
        // Get contracts
        RocketMinipoolManagerInterface rocketMinipoolManager = RocketMinipoolManagerInterface(
    ...
    function distributeBalanceAndFinalise()
    external
    override
    onlyInitialised
    onlyMinipoolOwnerOrWithdrawalAddress(msg.sender)
    {
        // Can only call if withdrawable and can only be called once
        require(status == MinipoolStatus.Withdrawable, 'Minipool must be withdrawable');
    ...
    function distributeBalance() external override onlyInitialised {
        // Must be called while staking or withdrawable
        require(
            status == MinipoolStatus.Staking || status == MinipoolStatus.Withdrawable,
        ...
        if (msg.sender != nodeAddress && msg.sender != nodeWithdrawalAddress) {
            ...
            if (status == MinipoolStatus.Staking) {
                ...
                require(totalBalance >= 3 ether, 'Balance must be greater than 3 AVAX');
            } else {
                // Then enough time must have elapsed
                require(
                    block.timestamp > statusTime.add(14 days),
    ...
    function _finalise() private {
        ...
        // Send any left over ETH to ggpAVAX contract
        if (address(this).balance > 0) {
            // Send user amount to ggpAVAX contract
            payable(gogoTokenGGPAVAX).transfer(address(this).balance);
        }
        // Trigger a deposit of excess collateral from ggpAVAX contract to deposit pool
        GoGoTokenGGPAVAXInterface(gogoTokenGGPAVAX).depositExcessCollateral();
        // Unlock node operator's GGP
    ...
    function _distributeBalance(uint256 _balance) private {
        ...
        require(
            block.number > withdrawalBlock + distributionCooldown,
        ...
        // Pay user amount to ggpAVAX contract
        if (userAmount > 0) {
            // Send user amount to ggpAVAX contract
            payable(gogoTokenGGPAVAX).transfer(userAmount);
        }
    ...
    function calculateNodeShare(uint256 _balance) public view override returns (uint256) {
        // Get fee and balances from minipool contract
        uint256 stakingDepositTotal = 6 ether;
    ...
    // Given a validator balance, this function returns what portion of it belongs to ggpAVAX users taking into consideration
    // the minipool's commission rate and any penalties it may have attracted
    ...
    function dissolve() external override onlyInitialised {
        // Check current status
        require(
            status == MinipoolStatus.Initialised || status == MinipoolStatus.Prelaunch,
    ...
    function close() external override onlyMinipoolOwner(msg.sender) onlyInitialised {
        // Check current status
    ...
    function voteScrub() external override onlyInitialised {
        ...
        uint256 quorum = rocketDAONode.getMemberCount().mul(rocketDAONodeTrustedSettingsMinipool.getScrubQuorum()).div(
            calcBase
        );
        if (totalScrubVotes.add(1) > quorum) {
            // Dissolve this minipool, recycling ETH back to deposit pool
            _dissolve();
            // Slash GGP equal to minimum stake amount (if enabled)
            if (rocketDAONodeTrustedSettingsMinipool.getScrubPenaltyEnabled()) {
                ...
                RocketDAOProtocolSettingsNodeInterface rocketDAOProtocolSettingsNode = RocketDAOProtocolSettingsNodeInterface(
                    getContractAddress('rocketDAOProtocolSettingsNode')
                );
                RocketDAOProtocolSettingsMinipoolInterface rocketDAOProtocolSettingsMinipool = RocketDAOProtocolSettingsMinipoolInterface(
                    getContractAddress('rocketDAOProtocolSettingsMinipool')
                );
                rocketNodeStaking.slashGGP(
                    nodeAddress,
    ...
    // Slash node operator's GGP balance based on nodeSlashBalance
    function _slash() private {
        ...
        nodeSlashBalance = 0;
        rocketNodeStaking.slashGGP(nodeAddress, slashAmount);
    }
