...
import "../../interface/minipool/RocketMinipoolLinkedQueueInterface.sol";
...
contract RocketMinipoolLinkedQueue is RocketBase, RocketMinipoolLinkedQueueInterface {
    ...
    // Constants
    bytes32 private constant queueKeyFull = keccak256("linked.minipools.available.full");
    bytes32 private constant queueKeyHalf = keccak256("linked.minipools.available.half");
    bytes32 private constant queueKeyVariable = keccak256("linked.minipools.available.variable");
    ...
    function getLengthLegacy(bytes32 _key) private view returns (uint256) {
        AddressLinkedQueueStorageInterface addressLinkedQueueStorage = AddressLinkedQueueStorageInterface(getContractAddress("addressLinkedQueueStorage"));
        return addressLinkedQueueStorage.getLength(_key);
    }

    /// @notice Gets the length of the variable (global) queue
    function getLength() override public view returns (uint256) {
        AddressLinkedQueueStorageInterface addressLinkedQueueStorage = AddressLinkedQueueStorageInterface(getContractAddress("addressLinkedQueueStorage"));
        return addressLinkedQueueStorage.getLength(queueKeyVariable);
    }
    ...
    function enqueueMinipool(address _minipool) override external onlyLatestContract("rocketMinipoolQueue", address(this)) onlyLatestContract("rocketNodeDeposit", msg.sender) {
        // Enqueue
        AddressLinkedQueueStorageInterface addressLinkedQueueStorage = AddressLinkedQueueStorageInterface(getContractAddress("addressLinkedQueueStorage"));
        addressLinkedQueueStorage.enqueueItem(queueKeyVariable, _minipool);
        // Emit enqueued event
    ...
    function dequeueMinipool(bytes32 _key) private returns (address) {
        // Dequeue
        AddressLinkedQueueStorageInterface addressLinkedQueueStorage = AddressLinkedQueueStorageInterface(getContractAddress("addressLinkedQueueStorage"));
        address minipool = addressLinkedQueueStorage.dequeueItem(_key);
        // Emit dequeued event
    ...
    function removeMinipool(bytes32 _key, address _minipool) private {
        // Remove
        AddressLinkedQueueStorageInterface addressLinkedQueueStorage = AddressLinkedQueueStorageInterface(getContractAddress("addressLinkedQueueStorage"));
        addressLinkedQueueStorage.removeItem(_key, _minipool);
        // Emit removed event
    ...
    function getMinipoolAt(uint256 _index) override external view returns(address) {
        ...
        uint256 halfLength = addressLinkedQueueStorage.getLength(queueKeyHalf);
        if (_index < halfLength) {
            return addressLinkedQueueStorage.getItem(queueKeyHalf, _index);
        }
        _index = _index.sub(halfLength);

        // Check if index is in the full queue
        uint256 fullLength = addressLinkedQueueStorage.getLength(queueKeyFull);
        if (_index < fullLength) {
            return addressLinkedQueueStorage.getItem(queueKeyFull, _index);
        }
        _index = _index.sub(fullLength);

        // Check if index is in the full queue
        uint256 variableLength = addressLinkedQueueStorage.getLength(queueKeyVariable);
        if (_index < variableLength) {
            return addressLinkedQueueStorage.getItem(queueKeyVariable, _index);
        }
    ...
    function getMinipoolPosition(address _minipool) override external view returns (int256) {
        ...
        // Check in half queue
        position = addressLinkedQueueStorage.getIndexOf(queueKeyHalf, _minipool);
        if (position != -1) {
            return position;
        }
        int256 offset = SafeCast.toInt256(addressLinkedQueueStorage.getLength(queueKeyHalf));

        // Check in full queue
        position = addressLinkedQueueStorage.getIndexOf(queueKeyFull, _minipool);
        if (position != -1) {
            return offset.add(position);
        }
        offset = offset.add(SafeCast.toInt256(addressLinkedQueueStorage.getLength(queueKeyFull)));

        // Check in variable queue
        position = addressLinkedQueueStorage.getIndexOf(queueKeyVariable, _minipool);
        if (position != -1) {
            return offset.add(position);
        }
