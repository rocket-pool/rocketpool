...
contract BeaconStateVerifier is RocketBase, BeaconStateVerifierInterface {
    ...
    function verifyValidator(ValidatorProof calldata _proof) override external view returns(bool) {
        ...
        bytes32 root = _getBlockRoot(_proof.slot);
    ...
    function verifyWithdrawal(WithdrawalProof calldata _proof) override external view returns(bool) {
        ...
        // Construct gindex
        SSZ.Path memory path = _pathBeaconBlockHeaderToStateRoot();
        path = SSZ.concat(path, _pathBeaconStateToPastBlockRoot(_proof.slot, _proof.withdrawalSlot));
    ...
    function _getBlockRoot(uint64 _slot) internal view returns (bytes32) {
    ...
    function _isHistoricalProof(uint64 proofSlot, uint64 targetSlot) internal view returns (bool) {
    ...
    function _merkleiseWithdrawal(Withdrawal calldata withdrawal) internal view returns (bytes32) {
    ...
    function _merkleiseValidator(Validator calldata validator) internal view returns (bytes32) {
