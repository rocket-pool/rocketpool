...
contract RocketPIP is RocketBase {
    ...
    */
    ...
    {
        // proposal must exist
        require(rocketStorage.getBool(keccak256(abi.encodePacked("rpip.proposal.exists", _proposalId))));
    ...
    */
    function submitProposal(uint _commitEnd, uint _revealEnd, uint _voteQuorum)
        public
        onlyRole("proposer") // Proposals can only be submitted by the proposer role
    {
     ...
     */
    function getCommitment(uint _proposalId, address _voterAddress)
        public
        view
        returns(bytes32, uint)
    {
        // commitment must exist
        require(rocketStorage.getBool(keccak256(abi.encodePacked("rpip.commitment.exists", _proposalId, _voterAddress))));
    ...
    */
    function commitVote(uint _proposalId, bytes32 _secretHash) 
        public
    {
     ...
     */
    function getVote(uint _proposalId, address _voterAddress)
        public
        view
        returns(bool, uint)
    {
        // revealed vote must exist
        require(rocketStorage.getBool(keccak256(abi.encodePacked("rpip.voter.exists", _proposalId, _voterAddress))));
     ...
     */
     function revealVote(uint _proposalId, bool _vote, uint _salt)
        public
    {
        // proposal must exist
        require(rocketStorage.getBool(keccak256(abi.encodePacked("rpip.proposal.exists", _proposalId))));
        ...
        require(rocketStorage.getBool(keccak256(abi.encodePacked("rpip.commitment.exists", _proposalId, msg.sender))));
        ...
        require(now >= commitEnd && now < revealEnd);
        ...
        require(keccak256(abi.encode(_proposalId, _vote, _salt)) == commitmentHash);
    ...
    */
    function getVotesFor(uint _proposalId)
        public
        view
        returns(uint)
    {
    ...
    * @param _proposalId Identifier of proposal
    */
    function getVotesAgainst(uint _proposalId)
        public
        view
        returns(uint)
    {
    ...
    * @param _proposalId Identifier of proposal
    */
    function isPassed(uint _proposalId)
        public
        view
        returns(bool)
    {
