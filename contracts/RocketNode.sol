...
contract RocketNode is RocketBase {
    ...
    /// @param _sig Signature generated by signing a hash of the valCodeAddress,
    function nodeAdd(address _newNodeAddress, string _providerID, string _subnetID, string _instanceID, string _regionID, address _valCodeAddress, bytes _sig) public onlyOwner returns (bool) {
        // Get our settings
        ...
        // Check the nodeAddress code address is legit and has signed the message which is a hashed version of the address prepended by the string 'Ethereum Signed Message' which is added onto signing messages by web3/nodes for ethereum etc.
        require(_newNodeAddress == rocketUtils.sigVerifyRecoverAddr(keccak256("\x19Ethereum Signed Message:\n32", keccak256(_valCodeAddress)), _sig));
        // Get our casper validation contract
        caperValidation = Validation(_valCodeAddress);
        // Check the validation code address is ok, must match the node owner that created that validation contract
        require(_newNodeAddress == caperValidation.nodeOwner());
